...
export default function UserDashboard() {
    ...
    async function checkIn() {
        try {
            const response = await axios.post(`https://attendance-tracker-backend-5x8m.onrender.com/checkIn`, {}, { withCredentials: true });
    ...
    async function checkOut() {
        try {
            const response = await axios.post(`https://attendance-tracker-backend-5x8m.onrender.com/checkOut`, {}, { withCredentials: true });
            if (response.data.success) {
                setCheckedOut(true);
                // Convert the UTC date string to a JavaScript Date object
